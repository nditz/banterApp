...
/** Country explicitly chosen at terms consent (null until then). */
...
/** Browser locale hint for guest previews only — never written to storage. */
...
/** Stored consent choice, else browser hint for guest API previews. */
export function detectCountryCode(): string {
  return getStoredCountryCode() ?? detectBrowserCountryCode();
}
